...
func _ready() -> void:
	...
	var controllers_in_scene = get_tree().get_root().get_nodes_in_group("FlowAIController")
	...
	for area in a_flowai_controller.areas:
		var shortest_dist:float = 0.0
		var dist = area.global_position.distance_to(a_body.global_position)
		if dist < shortest_dist:
			shortest_dist = dist
			nearest_area = area
	
	a_area = nearest_area
	a_astar = nearest_area.create_astar()
...
func get_random_path() -> void:
	...
	astar_find_path(start_node, goal_node)
...
func set_path(goal:FlowAIPathNode) -> void:
	...
	astar_find_path(start_node, goal)

func astar_find_path(start:FlowAIPathNode, goal:FlowAIPathNode):
	...
	if a_astar.has_point(start_id) and a_astar.has_point(goal_id):
		return a_astar.get_point_path(start_id, goal_id)
	else:
...
func get_next_pathnode_position() -> Vector3:
	...
	if a_path_index < a_current_path.size():
		return a_current_path[a_path_index].global_position
	else:
